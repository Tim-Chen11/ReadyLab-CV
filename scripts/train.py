...
class Trainer:
    ...
    def __init__(self, config):
        ...
        self.use_cuda = torch.cuda.is_available()  # Added this line
    ...
    def create_dataloaders(self):
        ...
        # Use small splits if in test mode
        if self.config.get('test_mode', False):
            train_split_name = 'train_small.json'
            val_split_name = 'val_small.json'
            logger.info("Using small test splits")
        else:
            train_split_name = 'train.json'
            val_split_name = 'val.json'

        # Construct absolute paths
        train_split_path = os.path.join(project_root, 'data', 'splits', train_split_name)
        val_split_path = os.path.join(project_root, 'data', 'splits', val_split_name)
        cache_dir = os.path.join(project_root, 'data', 'cache', 'images')

        # Create datasets (rest of your existing code)
        if dataset_class == URLDataset:
            train_dataset = URLDataset(
                split_file=train_split_path,
                transform=train_transform,
                cache_dir=self.config.get('cache_dir', cache_dir)
            )
        ...
        # Determine optimal settings based on device - FIXED SECTION
        if self.use_cuda:
            num_workers = self.config.get('num_workers', 4)
            pin_memory = True
        else:
            # Reduce workers and disable pin_memory for CPU
            num_workers = min(2, self.config.get('num_workers', 4))
            pin_memory = False
            if self.config.get('num_workers', 4) > 2:
                logger.info("Reduced num_workers to 2 for CPU training")

        # Create dataloaders - FIXED SECTION
    ...
    def train_epoch(self, model, dataloader, criterion, optimizer, scaler, scheduler=None):
        """Train for one epoch"""
        model.train()
        ...
        for batch_idx, (images, labels, _) in enumerate(pbar):
            ...
            # Mixed precision training - FIXED SECTION
            use_amp = self.config.get('use_amp', True) and self.use_cuda

            with autocast(device_type='cuda' if self.use_cuda else 'cpu', enabled=use_amp):
                outputs = model(images)
                loss = criterion(outputs, labels)

            # Backward pass - FIXED SECTION
    ...
    def train(self):
        ...
        # Only create scaler if using CUDA and AMP - FIXED SECTION
        use_amp = self.config.get('use_amp', True) and self.use_cuda
        scaler = GradScaler(device='cuda') if use_amp else None

        if not self.use_cuda and self.config.get('use_amp', True):
            logger.info("AMP disabled - CUDA not available")
        ...
        for epoch in range(self.config['epochs']):
            ...
            if epoch > 0 and scheduler:
                if isinstance(scheduler, torch.optim.lr_scheduler.ReduceLROnPlateau):
                    # ReduceLROnPlateau should still be called after validation
                    pass
        ...
        # Confusion matrix with zero_division fix - FIXED SECTION
        cm = confusion_matrix(labels, predictions)
        report = classification_report(
            labels,
            predictions,
            target_names=decade_names,
            zero_division=0  # This fixes the sklearn warning
        )
